...
void VertexClass::computeEnergyForce(double* x, double* y, double* z, 
	...
	if (candidate) {
		...
		if (deviation_distance) {
			...
			D = sqrt(outward_normal[0]*outward_normal[0]
								+outward_normal[1]*outward_normal[1]
								+outward_normal[2]*outward_normal[2]);
			scaled_spring_force = SEPARATION_WEIGHT*position_error/D;
			Cx = -outward_normal[0];
			Cy = -outward_normal[1];
			Cz = -outward_normal[2];
/*			cout << "\n\nVertexClass::computeEnergyForce: WARNING!! deviation_distance = 0.\n\n";
			cout << "VertexClass::computeEnergyForce: "
				<< "closest vertex = "
				<< closest[0] << " "
				<< closest[1] << " "
				<< closest[2] << endl;
...
//		force[2] = scaled_spring_force*deviation[2];
		force[0] = scaled_spring_force*Cx;
		force[1] = scaled_spring_force*Cy;
		force[2] = scaled_spring_force*Cz;
...
VertexClass::~VertexClass(void) {
	...
	for (i=0;i<HASH_TABLE_SIZE;i++) {
		delete[] neighborhood_vertices_index[i];
	}
}
...
void ObjectClass::fileInit(void) {
	...
	Objectfile << "OBJECT\n";
	Objectfile << "name = " << name << endl;
	Objectfile << "index = " << object_index << endl;
	Objectfile << "(#edges,#vertices,#polygons) = (" 
				<< num_edges << ","<< num_vertices << "," << num_polygons << ")" << endl;
...
void ObjectClass::InitVertices(void) {
	...
	for (i=0;i<num_vertices;i++) {
		...
		if ((object_index==WATCH_OBJECT) && vertex_print && WATCH_VERTICES) {
...
void ObjectClass::initializeNewCoordinatesAndDevDist(void) {
    ...
    for (j = 0;j < num_vertices;j++) {
		...
		vertices[j].hashval = computeHashValue(j);
	}
}
...
void ObjectClass::findNeighborhoodVertices(void) {
	...
	int neighbor_vertex;
	int num_ints;
	unsigned int* adjacent_bits = NULL;
	unsigned int* mask = NULL;
	unsigned int* new_bits = NULL;
	int uint_num;
	int bit_num;
	int q;

	unsigned int power_of_two[32] = { 1, 2, 4, 8, 16, 32, 64, 128, 
							256, 512, 1024, 2048, 4096, 8192, 
							16384, 32768, 65536, 131072, 262144, 
							524288, 1048576, 2097152, 4194304, 
							8388608, 16777216, 33554432, 67108864, 
							134217728, 268435456, 536870912, 
							1073741824, 0x80000000};
...
//	cout << "mean = " << mean_edge_length << ", radius = " << radius << endl;
...
//		cout << "find neighbors: vertex " << i << " of " << num_vertices << endl;
	    ...
	    for (j=0;j<HASH_TABLE_SIZE;j++) {
			vertices[i].neighborhood_vertices_index[j] = new int[NUMBER_OF_NEIGHBORS];
			if (vertices[i].neighborhood_vertices_index[j] == NULL) { 
				cout << "Not enough memory for neighborhood_vertices_index "
					<< "in ObjectClass::findNeighborhoodVertices\n";
				cout.flush();
				exit(1);
			}
			vertices[i].max_neighborhood_vertices_index[j] = NUMBER_OF_NEIGHBORS;
			vertices[i].next_neighbor_vertex[j] = 0;
		}

        if (temp != NULL) {
            delete [] temp;
        }
        temp = new int [NUMBER_LIST];
        if (temp == NULL) {
        ...
        num_temp = 0;
        max_temp = NUMBER_LIST;


		// initialize bit arrays
		num_ints = (int) ceil((double)num_vertices/32);
        if (adjacent_bits != NULL) {
            delete [] adjacent_bits;
        }
        adjacent_bits = new unsigned int [num_ints];
        if (adjacent_bits == NULL) {
            cout << "Not enough memory for adjacent_bits (first) in ObjectClass::findNeighborhoodVertices\n";
            cout.flush();
            exit(1);
        }
        ...
        mask = new unsigned int [num_ints];
        if (mask == NULL) {
            cout << "Not enough memory for mask (first) in ObjectClass::findNeighborhoodVertices\n";
            cout.flush();
            exit(1);
        }
        ...
        new_bits = new unsigned int [num_ints];
        if (new_bits == NULL) {
            cout << "Not enough memory for new_bits (first) in ObjectClass::findNeighborhoodVertices\n";
            cout.flush();
            exit(1);
        }
		...
		addArrayElement(vertices[i].neighborhood_vertices_index[computeHashValue(i)], 
						vertices[i].next_neighbor_vertex[computeHashValue(i)],
						vertices[i].max_neighborhood_vertices_index[computeHashValue(i)], 
						INCREMENT_OF_NEIGHBORS,i);

		// initialize adjacent_bits	
		uint_num = i/32;
		bit_num = i-32*uint_num;
		adjacent_bits[uint_num] = power_of_two[bit_num];

		////// build neighborhood //////
		for (j=0;j<radius;j++) {

			// adjacent_bits AND mask
			for (k=0;k<num_ints;k++) {
				new_bits[k] = adjacent_bits[k]&mask[k];
			}

			// mask AND NOT new_bits
			for (k=0;k<num_ints;k++) {
				mask[k] = mask[k]&~new_bits[k];
			}

			// reset adjacent bits to all zeros
			for (k=0;k<num_ints;k++) {
				adjacent_bits[k] = 0;
			}
			...
			for (k=0;k<num_ints;k++) {
				...
				if (new_bits[k] != 0 ) {
					// check all 32 bits
					for (q=0;q<32;q++) {
						if (power_of_two[q]&new_bits[k]) {
							// save new vertex index
							addArrayElement(temp,num_temp,max_temp,INCREMENT_OF_LIST,32*k+q);
						}
					}
				}
			}
			...
			for (k=0;k<num_temp;k++) {
				// add to neighborhood
				addArrayElement(vertices[i].neighborhood_vertices_index[vertices[temp[k]].hashval], 
								vertices[i].next_neighbor_vertex[vertices[temp[k]].hashval],
								vertices[i].max_neighborhood_vertices_index[vertices[temp[k]].hashval], 
								INCREMENT_OF_NEIGHBORS,temp[k]);
				///// save adjacent vertices as adjacent /////
				// for each adjacent vertex of new vertex
			    for (q=0;q<vertices[temp[k]].next_adjacent_vertex;q++) {

					// get adjacent vertex
					adjacent = vertices[temp[k]].adjacent_vertices_index[q];

					// add 1 to corresponding bit location in adjacent
					uint_num = adjacent/32;
					bit_num = adjacent-32*uint_num;
					adjacent_bits[uint_num] = adjacent_bits[uint_num]|power_of_two[bit_num];
				}
			}
			...
			temp = new int [NUMBER_LIST];
			if (temp == NULL) { 
				cout << "Not enough memory for temp (second) in ObjectClass::findNeighborhoodVertices\n";
				cout.flush();
				exit(1);
			}
			max_temp = NUMBER_LIST;
...
void ObjectClass::updateVertexLog(int iteration) {
	...
	// initialize vertex indices
	for (i=0;i<num_vertices;i++) {
		if (vertex_print && WATCH_VERTICES) {
...
void ObjectClass::boundObject(double* range) {
	...
	xmin = vertices[0].new_coordinates[0];
	xmax = vertices[0].new_coordinates[0];
	ymin = vertices[0].new_coordinates[1];
	ymax = vertices[0].new_coordinates[1];
	zmin = vertices[0].new_coordinates[2];
	zmax = vertices[0].new_coordinates[2];
	...
	for (j=0;j < num_vertices;j++) {
		...
		///////// extract coordinates //////////
		x = vertices[j].new_coordinates[0];
		y = vertices[j].new_coordinates[1];
		z = vertices[j].new_coordinates[2];
...
void ContainerClass::updateObjectAndVertexLog(int iteration) {
	...
	for (k=0;k<object_count;k++) {
		...
		if ((k==WATCH_OBJECT) && vertex_print) {
...
void ContainerClass::objectList(void) {

	char file[128];
	int k;
	sprintf(file,"%s%s",OUTPUT_DATA_DIR,OBJECT_LIST_FILE);
	ObjectList.open(file);
    ObjectList << "Object name 		Object index\n";

	for (k=0;k<object_count;k++) {
    	ObjectList << objects[k].name << "		" << objects[k].object_index << "\n";
	}
	ObjectList.close();
...
void ScanClass::buildMeshAfter(ContainerClass& container ,PolygonClass *polygons,int iteration) {
	...
	for(i=0;i<object_count;i++) {
		...
		// create output filename
		if (append_iteration_number) {
			sprintf(file,"%s%s_PRIMPED_%i.mesh",OUTPUT_DATA_DIR,container.objects[i].name,iteration);
		} else {
			sprintf(file,"%s%s_PRIMPED.mesh",OUTPUT_DATA_DIR,container.objects[i].name);
		}
...
void SpaceClass::boundWorld(ContainerClass& container, int object_count) {
	...
	//initialize mins and maxes
	xmin = container.objects[0].vertices[0].new_coordinates[0];
	xmax = container.objects[0].vertices[0].new_coordinates[0];
	ymin = container.objects[0].vertices[0].new_coordinates[1];
	ymax = container.objects[0].vertices[0].new_coordinates[1];
	zmin = container.objects[0].vertices[0].new_coordinates[2];
	zmax = container.objects[0].vertices[0].new_coordinates[2];
...
//        for (j=0;j < container.objects[i].num_vertices;j++) {
            ...
            ////////// extract coordinates //////////
//			x = container.objects[i].vertices[j].new_coordinates[0];
//			y = container.objects[i].vertices[j].new_coordinates[1];
//			z = container.objects[i].vertices[j].new_coordinates[2];
...
void ManipClass::collectNicePolygons(ContainerClass& container, PolygonClass *polygons, SpaceClass &space, 
	...
	int box_range[6], target_axis, x, y, z, loc_z, loc_zy,num, i, j, k;
	bool not_adjacent;
	double coordinates[3];
	int* boxes_to_check;
	int num_boxes_to_check;
	int pi; // polygon_index
	int limit;
...
//			// add polygon if not adjacent to current vertex
			pi = space.boxes[boxes_to_check[j]].polygons[i];
			not_adjacent = true;
			for (k=0;k<container.objects[object_index].vertices[vertex_index].next_adjacent_polygon;k++) {
				if (pi == container.objects[object_index].vertices[vertex_index].adjacent_polygons_index[k]) {not_adjacent=false;}
			}
			if (not_adjacent) {
...
//								INCREMENT_OF_UNIQUE_POLYGONS,space.boxes[boxes_to_check[j]].polygons[i]);
				addArrayElement(unique_polygons,num_unique_polygons,max_unique_polygons,
								INCREMENT_OF_UNIQUE_POLYGONS,pi);
			}
		}
...
int ManipClass::checkPolygonPolygonIntersections(ContainerClass &container ,PolygonClass *polygons,
	...
	int i, j, k, object_index, other_object_index, opvi[3], cpvi[3], current_pairs[3][2], num_unique,single_shared_vert;
							...
							if (polygons[space.boxes[loc].polygons[k]].object_index == coi) {
								...
								for (p=0;p<3;p++) {
									if (safe_flag) {
										...
										hashval = container.objects[coi].vertices[poly_vertex].hashval;

										// get current vertex number of neighbor vertices
										nnv = container.objects[coi].vertices[cvi].next_neighbor_vertex[hashval];

										// look for polygon vertex index in sorted neighborhood list
										m = 0;
										search_flag = true;
										while (m<nnv && search_flag) {
											neighbor_vertex = 	container.objects[coi].vertices[cvi].neighborhood_vertices_index[hashval][m];
