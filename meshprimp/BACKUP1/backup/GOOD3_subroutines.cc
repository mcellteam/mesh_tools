		...
		for (j=0;j<radius;j++) {
			...
			for (a=0;a<num_list_last;a++) {
			    ...
			    for (k=0;k<vertices[list_last[a]].next_adjacent_vertex;k++) {
					...
					// get adjacent vertex
					adjacent = vertices[list_last[a]].adjacent_vertices_index[k];

					// compute adjacent hash value
//					hashval = computeHashValue(adjacent);
					hashval = vertices[adjacent].hashval;

					// get number of neighbor vertices for current last vertex
					nnv = vertices[i].next_neighbor_vertex[hashval];

					// look for adjacent vertex index in neighborhood list
					m = 0;
					search_flag = true;
					while (m<nnv && search_flag) {
						neighbor_vertex = vertices[i].neighborhood_vertices_index[hashval][m];
						if (adjacent == neighbor_vertex) {search_flag = false;}
						m++;
					}

					// if not found
					if (search_flag) {
						// add adjacent vertex to neighborhood
						addArrayElement(vertices[i].neighborhood_vertices_index[hashval], 
										vertices[i].next_neighbor_vertex[hashval],
										vertices[i].max_neighborhood_vertices_index[hashval], 
										INCREMENT_OF_NEIGHBORS,adjacent);
						// add adjacent vertex to temp array
						addArrayElement(temp,num_temp,max_temp,INCREMENT_OF_LIST,adjacent);
					}
