...
{
    if(left < right) {
        int split_pt = Partition_double(d,left, right);
        Quicksort_double(d, left, split_pt);
        Quicksort_double(d, split_pt+1, right);
    }
}
...
int Partition_double( double d[], int left, int right)
{
    ...
    int rm = right+1;
    for(;;) {
        do { rm--; } while (d[rm] > val);
        do { lm++; } while( d[lm] < val);
        if(lm < rm) {
            double tempr = d[rm];
            d[rm] = d[lm];
            d[lm] = tempr;
...
void addSortUniqueArrayElement(int*& ptr,int& num,int& max,int incr,int val) {
	...
	if (num==0) {
        ...
        int j = 0;
        while (j<num && (val>ptr[j])) { j++; }

		// j = insert location of val into ptr
		// is j location in ptr available?
		if (j == num) {
			// insert val into ptr
			addArrayElement(ptr,num,max,incr,val);
		} else {
			// if val equals ptr[j] do nothing
			// else insert val into ptr
			if (val != ptr[j]) {
...
void PolygonClass::checkEdgeEdgeIntersection(	double* cpvc[3], int current_pairs[3][2], 
	...
	// do polygons intersect?
	if (intersect_flag) {
		addArrayElement(intersections,num_intersections,max_intersections,
						INCREMENT_OF_INTERSECTIONS,other_polygon_index);
		self_intersection = true;
		cout << "checkEdgeEdgeIntersection: intersection was found!"
		<< ", current polygon index " << current_polygon_index
		<< ", intersecting polygon index " << other_polygon_index << "\n\n";
	}
}
...
void PolygonClass::checkPolygonEdgePolygonIntersection(	double* cpvc[3],
	...
	if (line_flag && poly_flag) {
		addArrayElement(intersections,num_intersections,max_intersections,
						INCREMENT_OF_INTERSECTIONS,other_polygon_index);
		self_intersection = true;
		cout << "checkPolygonEdgePolygonIntersection: intersection was found!"
		<< ", current polygon index " << current_polygon_index
		<< ", intersecting polygon index " << other_polygon_index << "\n";
	}
}
...
void EdgeClass::computeAngleForceEnergy(PolygonClass *polygons) {
	...
	double n[2][3], dot_product, mean[3], magn[2], force_magn, normal_length[2],
	...
	n[1][1] = polygons[polygon_indices[1]].normal_components[1];
	n[1][2] = polygons[polygon_indices[1]].normal_components[2];
	normal_length[0] = sqrt(n[0][0]*n[0][0]+n[0][1]*n[0][1]+n[0][2]*n[0][2]);
	normal_length[1] = sqrt(n[1][0]*n[1][0]+n[1][1]*n[1][1]+n[1][2]*n[1][2]);

	//// force direction for other_vertex_indices[0]////
	// normals cross product = n[0] x n[1]
	norm_cross[0] = n[0][1]*n[1][2]-n[0][2]*n[1][1];
	norm_cross[1] = n[0][2]*n[1][0]-n[0][0]*n[1][2];
	norm_cross[2] = n[0][0]*n[1][1]-n[0][1]*n[1][0];
	...
	final_dot_product = force_dir[0]*n[0][0]+force_dir[1]*n[0][1]+force_dir[2]*n[0][2];
	...
	magn[1] = sqrt(n[1][0]*n[1][0]+n[1][1]*n[1][1]+n[1][2]*n[1][2]);
	// cosine of angle
	new_angle_cosine = dot_product/magn[0]/magn[1];
	...
	force[0][0] = force_magn/2*flip*n[0][0]/normal_length[0];
	force[0][1] = force_magn/2*flip*n[0][1]/normal_length[0];
	force[0][2] = force_magn/2*flip*n[0][2]/normal_length[0];
	force[1][0] = force_magn/2*flip*n[1][0]/normal_length[0];
	force[1][1] = force_magn/2*flip*n[1][1]/normal_length[0];
	force[1][2] = force_magn/2*flip*n[1][2]/normal_length[0];
			...
			if (j) {
				...
				while ((a < num_list_last) && (b < num_temp1)) {
					if      (temp1[b] < list_last[a]) {
						addArrayElement(temp2,num_temp2,max_temp1,INCREMENT_OF_LIST,temp1[b] );
						b++;
...
void ManipClass::searchForSelfIntersection(ContainerClass &container ,PolygonClass *polygons,
											SpaceClass &space) {

	int i, j, k, other_polygon_object_index,current_polygon_object_index;
	int* current_boxes;
	int* other_polygons;
	int num_other_polygons;
	int max_other_polygons;

	self_intersection = false;

	////////// search for intersections between polygons //////////
    // for each polygon structure in master polygons array
	for (j=0;j<polygon_count;j++) {

		if (print_flag) {
			cout << "searchForSelfIntersection: polygon index = " << j << "\n";
			cout.flush();
		}

		// get current polygon object index	
		current_polygon_object_index = polygons[j].object_index;

		// get boxes for current polygon
		current_boxes = new int[polygons[j].num_boxes];
		if (current_boxes == NULL) { 
			cout << "Not enough memory for current_boxes in ManipClass::searchForSelfIntersection\n";
			cout.flush();
			exit(1);
		}
		for(k=0;k<polygons[j].num_boxes;k++){
			current_boxes[k] = polygons[j].boxes[k];
		}

		// clear intersections vector
		if (polygons[j].intersections != NULL) {
			delete [] polygons[j].intersections;
		}
		polygons[j].intersections = new int[NUMBER_OF_INTERSECTIONS];
		if (polygons[j].intersections == NULL) { 
			cout << "Not enough memory for polygons[j].intersections in ManipClass::searchForSelfIntersection\n";
			cout.flush();
			exit(1);
		}
		polygons[j].num_intersections = 0;
		polygons[j].max_intersections = NUMBER_OF_INTERSECTIONS;

		////////// find all other polygons in boxes of current polygon //////////

		other_polygons = new int[NUMBER_OF_CROSSED_POLYGONS];
		if (other_polygons == NULL) { 
			cout << "Not enough memory for other_polygons in ManipClass::searchForSelfIntersection\n";
			cout.flush();
			exit(1);
		}
		num_other_polygons = 0;
		max_other_polygons = NUMBER_OF_CROSSED_POLYGONS;

		// for each box
		for(k=0;k<polygons[j].num_boxes;k++){
			// for each polygon in box

			for (i=0;i<space.boxes[current_boxes[k]].num_polygons;i++) {

				// get other polygon object index	
				other_polygon_object_index = polygons[space.boxes[current_boxes[k]].polygons[i]].object_index;

				if ((space.boxes[current_boxes[k]].polygons[i] != j) ||
					(other_polygon_object_index == current_polygon_object_index)) {
					addSortUniqueArrayElement(other_polygons,num_other_polygons,max_other_polygons,
									INCREMENT_OF_CROSSED_POLYGONS,space.boxes[current_boxes[k]].polygons[i]);
				}
			}
		}

		////////// look for and record intersections //////////
		////////// between current polygon and other_polygons //////////

		checkPolygonPolygonIntersections(container, polygons, j, other_polygons,num_other_polygons);

		// free memory
		delete[] current_boxes;
		delete[] other_polygons;
...
void ManipClass::checkPolygonPolygonIntersections(ContainerClass &container ,PolygonClass *polygons,
													int cpi,int* other_polygons, int num_other_polygons) {
	...
	// for each other_polygon
	for (i=0;i<num_other_polygons;i++) {

		// get other_polygon object index
		other_object_index = polygons[other_polygons[i]].object_index;

		// get other_polygon normal
		on[0] = polygons[other_polygons[i]].normal_components[0];
		on[1] = polygons[other_polygons[i]].normal_components[1];
		on[2] = polygons[other_polygons[i]].normal_components[2];

		// get other_polygon vertex indices
		opvi[0] = polygons[other_polygons[i]].vertices[0] ;
		opvi[1] = polygons[other_polygons[i]].vertices[1] ;
		opvi[2] = polygons[other_polygons[i]].vertices[2] ;

		// get other_polygon vertex coordinates
		container.objects[other_object_index].getPolygonCoordinates(polygons[other_polygons[i]].vertices, opvc);
		...
		if (coplanar_flag) {
			if (identical_flag) {
				// polygons intersect
				addArrayElement(polygons[cpi].intersections,polygons[cpi].num_intersections,
								polygons[cpi].max_intersections, INCREMENT_OF_INTERSECTIONS,other_polygons[i]);
				self_intersection = true;
				cout << "checkPolygonPolygonIntersections: intersection was found!"
				<< ", current polygon index " << cpi
				<< ", intersecting polygon index " << other_polygons[i] << "\n";
			} else {
				//do polygon edges intersect?	
				// if yes, intersect
				// if no, do not intersect
				polygons[cpi].checkEdgeEdgeIntersection(cpvc, current_pairs, opvc,
														cpi, other_polygons[i], share_edge_flag,
														self_intersection);
			}
		} else {
			if (!share_edge_flag) {
				// do polygons intersect?
				polygons[cpi].checkPolygonEdgePolygonIntersection( cpvc, current_pairs, on,  
																	opvc, cpi, other_polygons[i],
																	self_intersection);
			}
		}
	}
...
void ManipClass::findClosestVertex(ContainerClass& container ,PolygonClass *polygons, SpaceClass &space, 
	...
	int cbi[3], offset, box_range[6], x, y, z, closest_polygon_index , loc_z, loc_zy, loc,i, j, k, m;
		...
		if (!closest_flag) {
					...
					for (x = box_range[0];x<box_range[1]+1;x++){
						...
						for (k=0;k<space.boxes[loc].num_polygons;k++) {
							...
							if (polygons[space.boxes[loc].polygons[k]].object_index == coi) {
								...
								// for each neighborhood vertex
							    for (m=0;m<container.objects[coi].vertices[cvi].next_neighbor_vertex;m++) {
									if (
										(container.objects[coi].vertices[cvi].neighborhood_vertices_index[m] 
											== polygons[space.boxes[loc].polygons[k]].vertices[0]) ||
										(container.objects[coi].vertices[cvi].neighborhood_vertices_index[m] 
											== polygons[space.boxes[loc].polygons[k]].vertices[1]) ||
										(container.objects[coi].vertices[cvi].neighborhood_vertices_index[m] 
											== polygons[space.boxes[loc].polygons[k]].vertices[2])
										) { safe_flag = false;}
								}
							}
							if (safe_flag) { 
			...
			if (candidates_flag){
				...
				for (j=0;j<count;j++) {
					...
					if (
						(!container.objects[coi].vertices[cvi].nice_data.nice && (dotproduct < 0)) || !dotproduct ){
...
void ManipClass::computeClosest(ContainerClass& container ,PolygonClass *polygons, SpaceClass &space,
				int polygon_index, double *current_vertex_coordinates, double *closest_vertex_coordinates) {
	...
	////////// compute minimum distance between current vertex and each polygon edge//////////
	...
	////////// compute minimum distance between current vertex and each polygon vertex//////////
	...
	for (i=0;i<3;i++) {
		closest_vertex_coordinates[i] = candidates[store][i];
	}
