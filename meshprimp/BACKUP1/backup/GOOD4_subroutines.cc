...
//		cout << "find neighbors: vertex " << i << " of " << num_vertices << endl;
		...
		uint_num = (int)floor((double)i/32);
		...
		for (j=0;j<radius;j++) {
			...
			for (k=0;k<num_temp;k++) {
			    ...
			    for (q=0;q<vertices[temp[k]].next_adjacent_vertex;q++) {
					...
					uint_num = (int)floor((double)adjacent/32);
...
//										nnv = container.objects[coi].vertices[cvi].next_neighbor_vertex[computeHashValue(poly_vertex)];
...
//											neighbor_vertex = 	container.objects[coi].vertices[cvi].neighborhood_vertices_index[computeHashValue(poly_vertex)][m];
						...
						if (temp<(SEARCH_RADIUS*SEARCH_RADIUS)) {
							...
							if (temp<squareD || !squareD) {
								...
								for (i=0;i<3;i++) {
									...
									if (closest_coordinates[i] == container.objects[coi].vertices[cvi].new_coordinates[i]){
										cout << "\nclosest = new\n";
										cout.flush();
										exit(1);
									}
