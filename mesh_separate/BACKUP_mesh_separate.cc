...
int main(int argc,char *argv[]){
  ...
  for (i=1;i<num_vertices+1;i++) {
    ...
    for (q=vertices[i];q!=NULL;q=q->next) {
      ...
      if (q->next!=NULL) {
        if (faces[(long)(q->data)]){
          current_group = faces[(long)q->data];
        }
      }
      if (q->next==NULL) break;
    }
    ...
    if (current_group) {
      ...
      for (q=vertices[i];q!=NULL;q=q->next) {
        // if not last link in list
        if (q->next!=NULL) {
          faces[(long)q->data] = current_group;
        }
        if (q->next==NULL) break;
      }
    } else {
  ...
  while(amalgamate) {
    ...
    for (i=1;i<num_vertices+1;i++) {
      ...
      swap = false;
      q=vertices[i];
      smallest_group = faces[(long)q->data];
      for (q=vertices[i];q!=NULL;q=q->next) {
        // look up face group
        // if not last link in list
        if (q->next!=NULL) {
          if (faces[(long)q->data]!=smallest_group){ swap=true;amalgamate = true;}
          if (faces[(long)q->data]<smallest_group){smallest_group=faces[(long)q->data];}
        }
        if (q->next==NULL) break;
      }
      ...
      if (swap) {
        ...
        for (q=vertices[i];q!=NULL;q=q->next) {
          ...
          if (q->next!=NULL) {
            ...
            if (faces[(long)q->data]!=smallest_group){
              //swap all occurences of group in faces array with smallest_group
              for (j=1;j<num_faces+1;j++) {
                if(faces[j]==faces[(long)q->data]) { faces[j]=smallest_group; }
              }
            }
          }
          if (q->next==NULL) break;
