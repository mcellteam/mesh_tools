...
void Container::statusFileInit(void) {
  ...
  Cfile.width(13);
  Cfile << left << "Duration(sec)";
  Cfile.width(11);
  Cfile << left << "Median ECW";
  Cfile.width(11);
  Cfile << left << "EVF" << endl;
  Cfile.flush();
}
...
double Face::computeArea(void)
{
  ...
  return sqrt(dot(n,n))/2.0;
}

double Container::computeECW(Space &s)
{
  cerr << "Compute extracellular width....................";
  cerr.flush();
  int iii=1;
  int TOTAL = vertex_count+face_count;
  double goal = 0.01;
  fprintf(stderr,"0%%..");
  fflush(stderr);
  ecw.clear();
  ...
  for(std::vector<Object*>::iterator i=o.begin();i!=o.end();i++)
  {
    ...
    for(std::vector<Vertex*>::iterator j=(*i)->v.begin();j!=(*i)->v.end();j++)
    {
      ...
      if((*j)->cl!=NULL && (binary_search(frozen.begin(),frozen.end(),*j)==false))
      {
        ...
        double pC[3];
        //computePC((*j)->cl,*j,pC);
        computePC((*j)->cl,(*j)->pN,pC);
        // compute separation vector
        double ss[3];
        for(int k=0;k<3;k++){ ss[k]=pC[k]-(*j)->pN[k]; }
        // save separation distance
        if((*i)->vertexIsNice(*j)){ ecw.push_back(sqrt(dot(ss,ss)));}
        else {ecw.push_back(-sqrt(dot(ss,ss)));}
      }
      // track progress
      double progress = static_cast<double>(iii++)/TOTAL;
      if(progress>goal){
    ...
    for(std::vector<Face*>::iterator j=(*i)->f.begin();j!=(*i)->f.end();j++)
    {
      ...
      if(m > 1.0)
      {
        ...
        for(int k=0;k<m;k++)
        {
          // compute barycenter
          double p[3],q[3];
          g.computeBarycenter(p,k,n,*j);
          // find closest point to barycenter
          if(findClosestBarycenter(s,p,q,*j)==true)
          {
            // measure ecw at barycenter
            double L = (p[0]-q[0])*(p[0]-q[0])+(p[1]-q[1])*(p[1]-q[1])+(p[2]-q[2])*(p[2]-q[2]);
            ecw.push_back(sqrt(L));
          }
        }
      }
      // track progress
  ...
  // return middle value
  sort(ecw.begin(),ecw.end());
  uint ss = ecw.size()/2;
  fprintf(stderr,"100%%..");
  fflush(stderr);
  cerr << "complete.\n";
  cerr.flush();
  if(ss<ecw.size()){return ecw[ss];}
  else {return -666;}
}

void Grid::computeBarycenter(double p[3],int index,int n,Face *f)
{
  ...
  p[0] = ucoef*unit_u[0] + vcoef*unit_v[0] + f->v[0]->pN[0];
  p[1] = ucoef*unit_u[1] + vcoef*unit_v[1] + f->v[0]->pN[1];
  p[2] = ucoef*unit_u[2] + vcoef*unit_v[2] + f->v[0]->pN[2];
}

Grid::Grid(Face *f)
{

  double fx = f->v[1]->pN[0] - f->v[0]->pN[0];
  double fy = f->v[1]->pN[1] - f->v[0]->pN[1];
  double fz = f->v[1]->pN[2] - f->v[0]->pN[2];
  double ff = 1 / sqrt( fx*fx + fy*fy + fz*fz );

  unit_u[0] = fx * ff;
  unit_u[1] = fy * ff;
  unit_u[2] = fz * ff;

  fx = f->v[2]->pN[0] - f->v[0]->pN[0];
  fy = f->v[2]->pN[1] - f->v[0]->pN[1];
  fz = f->v[2]->pN[2] - f->v[0]->pN[2];

  double normal[3];
  normal[0] = unit_u[1] * fz - unit_u[2] * fy;
  normal[1] = unit_u[2] * fx - unit_u[0] * fz;
  normal[2] = unit_u[0] * fy - unit_u[1] * fx;
  ff = 1 / sqrt( normal[0]*normal[0] + normal[1]*normal[1] + normal[2]*normal[2] );
  normal[0] *= ff;
  normal[1] *= ff;
  normal[2] *= ff;
  unit_v[0] = normal[1] * unit_u[2] - normal[2] * unit_u[1];
  unit_v[1] = normal[2] * unit_u[0] - normal[0] * unit_u[2];
  unit_v[2] = normal[0] * unit_u[1] - normal[1] * unit_u[0];

  uv_vert1_u = (f->v[1]->pN[0] - f->v[0]->pN[0])*unit_u[0] +
               (f->v[1]->pN[1] - f->v[0]->pN[1])*unit_u[1] +
               (f->v[1]->pN[2] - f->v[0]->pN[2])*unit_u[2];
  uv_vert2[0] = (f->v[2]->pN[0] - f->v[0]->pN[0])*unit_u[0] +
                (f->v[2]->pN[1] - f->v[0]->pN[1])*unit_u[1] +
                (f->v[2]->pN[2] - f->v[0]->pN[2])*unit_u[2];
  uv_vert2[1] = (f->v[2]->pN[0] - f->v[0]->pN[0])*unit_v[0] +
                (f->v[2]->pN[1] - f->v[0]->pN[1])*unit_v[1] +
                (f->v[2]->pN[2] - f->v[0]->pN[2])*unit_v[2];
}


double Object::computeVolume(void)
{
  double vol = 0.0;
  // for each face in object
  for(std::vector<Face*>::iterator i=f.begin();i!=f.end();i++)
...
double Container::computeEVF(void)
{
  cout << "Compute extracellular volume fraction..........";
  cout.flush();
  double intracellular_volume=0.0;
  // for each object in container
  for(std::vector<Object*>::iterator i=o.begin();i!=o.end();i++)
  {
    // compute object volume
    intracellular_volume += (*i)->computeVolume();
  }
  double evf = 1.0-intracellular_volume/WORLD_VOLUME;
  cout << "complete.\n";
  cout.flush();
  return evf;
}
...
void Container::updateFile(const int group,bool flag,double tim,double median_ecw, double evf) {
  if(group==0){
...
void Container::createEdges(void) {
  ...
  for (std::vector<Object*>::iterator i=o.begin();i!=o.end();i++) {
    if(WRITE_VERBOSE_INIT){
      cerr << "Creating edges for " << (*i)->name << "...";
      cerr.flush();
    }
    //		(*i)->createEdges();
    // determine number of digits in largest vertex index
...
void Container::findVertexAdjacencies(void) {
  if(!WRITE_VERBOSE_INIT){
    cerr << "Finding vertex adjacencies.....................";
    cerr.flush();
  }
  std::vector<Object*>::iterator i;
  // if 
  ...
  for (i=o.begin();i!=o.end();i++) {
    if(WRITE_VERBOSE_INIT){
      cerr << "Finding adjacencies for " << (*i)->name << "...";
      cerr.flush();
    }
...
void Container::computeECWstatistics(void)
{
  int n = ecw.size();
  // compute median of ecw
  double median = 0.0;
  // if ecw is not empyty
  if(ecw.empty()==false)
  {
    // if ecw has odd number of elements
    if(n%2==1)
    {
      uint mid = (n-1)/2+1;
      median = ecw[mid];
    }
    else // ecw has even number of elements
    {
      uint mid = n/2;
      median = (ecw[mid]+ecw[mid+1])/2;
    }
  }
  // compute mean and variance of ecw
  double sum=0.0,sum2=0.0;
  // for each element of ecw vector
  for(std::vector<double>::iterator i=ecw.begin();i!=ecw.end();i++)
  {
    sum+=*i;
    sum2+=(*i)*(*i);
  }
  ...
  cout.precision(9);
  cout << "median " << median << " mean " << mean << " variance " << variance << endl;
}
...
void Container::getCandidateFacesBarycenter(vector<Box*> &bp,Face *ff,std::vector<Face*> &cf)
{
  // for each box in search
  for (std::vector<Box*>::iterator j=bp.begin();j!=bp.end();j++) {
    // for each face in box
    for (std::vector<Face*>::iterator k=(*j)->f.begin();k!=(*j)->f.end();k++) {
      // if face is not same as face containing barycenter, ff
      if (*k!=ff){
        cf.push_back(*k);
      }
    }
...
void Container::getPD2(double pd2[6],double sr[6],double p[3]){
  // [xmin xmax ymin ymax zmin zmax]
  pd2[0] = (sr[0]-p[0])*(sr[0]-p[0]);
  pd2[1] = (sr[1]-p[0])*(sr[1]-p[0]);
  pd2[2] = (sr[2]-p[1])*(sr[2]-p[1]);
  pd2[3] = (sr[3]-p[1])*(sr[3]-p[1]);
  pd2[4] = (sr[4]-p[2])*(sr[4]-p[2]);
  pd2[5] = (sr[5]-p[2])*(sr[5]-p[2]);
}
...
bool Container::findClosest(Space &s,Vertex *v,Monitor& stats,bool flag) {
  if(binary_search(frozen.begin(),frozen.end(),v)==false){
    ...
    double vector3[3] ={v->pN[0],v->pN[1],v->pN[2]};
    getPD2(pd2,sr,vector3);
...
bool Container::findClosestBarycenter(Space &s,double p[3],double q[3],Face *ff)
{
  // p is barycenter, ff is parent face of p
  // squared distance between closest point and p
  double cd=0.0;
  // old and new Face* vectors
  std::vector<Face*> of,nf;
  // old and new Box* vectors
  ...
  //  sr limits [xmin xmax ymin ymax zmin zmax]
  double sr[6];
  //  6 cartesian principal distances squared (pd2) from p to sr limits
  // [xmin xmax ymin ymax zmin zmax]
  double pd2[6];
  // let search region (sr) be box containing barycenter, p
  // get box indices for sr : [xmin xmax ymin ymax zmin zmax]
  int indices[6] = { s.location2Index(p[0],"x"),s.location2Index(p[0],"x"),
    s.location2Index(p[1],"y"),s.location2Index(p[1],"y"),
    s.location2Index(p[2],"z"),s.location2Index(p[2],"z")};
  // get Box* for sr : Space::getBoxesFor3DLocations(v->pN)
  s.getBoxesFor3DIndices(indices,nb);
  // collect Face* in sr
  getCandidateFacesBarycenter(nb,ff,nf);
  // sort and keep unique Face*
  sort(nf.begin(),nf.end());
  std::vector<Face*>::iterator z = unique(nf.begin(),nf.end());
  nf.assign(nf.begin(),z);
  ...
  // search for closest point (cp) and closest distance (cd) among faces
  bool gate = false;
  if(searchCPBarycenter(p,ff,nf,cd,q)==true){gate=true;}
  // if sr is expanded, then reanalyze
  while(s.expandSR(gate,cd,pd2,inc,indices)==true){
    // copy Box* vector and get Box* for new sr
    ob = nb;
...
bool Container::searchCPBarycenter(double p[3],Face *ff,std::vector<Face*> &nf,double &squareD,double q[3])
{
  bool gate = false;
  ...
  if (!nf.empty()){
    ...
    if(verts.empty()==false || edges.empty()==false || faces.empty()==false){
      if(computeClosestCollBarycenter(verts,edges,faces,p,squareD,n,q,ff)){ gate=true;}
    }
    ...
    if(gate==false){
      ...
      while(gate==false && i!=df.end()){
        ...
        // if the closest point to current vertex was found on this face
        if(computeClosestBarycenter((*i).second,p,squareD,n,q)){ gate=true;}
        // increment iterator
        i++;
      }
    }
...
bool Container::getPlaneIntersection(Face *f,double pp[3],double *n,double num,double den,Point &p){
  //compute point on face plane that is closest to current vertex
  // i.e. intersection of plane with face normal through current vertex
  bool line_flag, poly_flag=false,poly_edge_flag;
  double u=num/den;
  double lp[2][3] = {{pp[0],pp[1],pp[2]},
      {pp[0]+u*n[0],pp[1]+u*n[1],pp[2]+u*n[2]}};
  checkLineFaceIntersection(f,lp,line_flag,poly_flag,poly_edge_flag);
  // if intersection point is on face,then save point
  if (poly_flag) {p.add(lp[1][0],lp[1][1],lp[1][2]);}
  //	return (poly_flag || poly_edge_flag);
...
void Container::getEdgeIntersection(const double pp[3],double *P[3],Point &p){
  // first pair of face vertices, P[0],P[1]
  double e[3]={P[1][0]-P[0][0],P[1][1]-P[0][1],P[1][2]-P[0][2]};
  double vv[3]={pp[0]-P[0][0],pp[1]-P[0][1],pp[2]-P[0][2]};
  double u = dot(vv,e)/dot(e,e);
  if (u>0 && u<1) {
    p.add(P[0][0]+u*(e[0]),P[0][1]+u*(e[1]),P[0][2]+u*(e[2]));
  }
...
Face* Container::getNonAdjacentFaceOfVertexBarycenter(Face *ff,Vertex *tv){
  // for each adjacent face of tv
  for(std::vector<Face*>::iterator i=tv->f.begin();i!=tv->f.end();i++){
    // if face is not equal to parent face, ff, of barycenter
    if(ff!=*i){
      return *i;
    }
  }
...
Face* Container::getNonAdjacentFaceOfEdgeBarycenter(Face *ff,Edge *ee){
  // if first edge face is not equal to parent face, ff, of barycenter
  if(ff!=ee->f1){
    return ee->f1;
  }
  // if second edge face is not equal
  if(ff!=ee->f2){
    return ee->f2;
  }
...
bool Container::computeClosestCollBarycenter(std::vector<Vertex*> &verts,std::vector<Edge*> &edges,
                                   std::vector<Face*> &faces,double p[3],double &squareD,double vn[3],double QQ[3],Face *ff)
{
  bool signal = false;
  ...
  for(std::vector<Vertex*>::iterator i=verts.begin();i!=verts.end();i++){
    ...
    double sqd = ((*i)->pN[0]-p[0])*((*i)->pN[0]-p[0])+
                 ((*i)->pN[1]-p[1])*((*i)->pN[1]-p[1])+
                 ((*i)->pN[2]-p[2])*((*i)->pN[2]-p[2]);
    // create triplet
    t = new Triplet((*i)->pN[0],(*i)->pN[1],(*i)->pN[2]);
    // add squared distance to fmap
    fmap[sqd]=getNonAdjacentFaceOfVertexBarycenter(ff,*i);
    // add vertex to tmap
  ...
  for(std::vector<Edge*>::iterator i=edges.begin();i!=edges.end();i++){
    ...
    if (u>0 && u<1) {
      ...
      // squared distance between v and i
      double sqd = (ix-p[0])*(ix-p[0])+
                   (iy-p[1])*(iy-p[1])+
                   (iz-p[2])*(iz-p[2]);
      // create triplet
      t = new Triplet(ix,iy,iz);
      // add squared distance to fmap
      fmap[sqd]=getNonAdjacentFaceOfEdgeBarycenter(ff,*i);
      // add vertex to tmap
      if(!tmap.count(sqd)){tmap[sqd]=t;}
    }
  }
  ...
  for(std::vector<Face*>::iterator i=faces.begin();i!=faces.end();i++){
    ...
    if (fabs(num)>DOUBLE_EPSILON) {
      ...
      // add to p if intersection is on face
      // return true if point lies inside or on an edge of face, false otherwise
      inside = getPlaneIntersection(*i,p,fn,num,dot(fn,fn),pp);
    }
    // if a point has been found, i.e. inside==true
    if(inside==true){
      // create triplet
      t = new Triplet(pp.a,pp.b,pp.c);
      // add squared distance to fmap
      fmap[pp.L]=*i;
      // add point to tmap
      if(!tmap.count(pp.L)){tmap[pp.L]=t;}
    } else {
    }
  }
  ...
  double vn_copy[3];
  vn_copy[0]=vn[0];vn_copy[1]=vn[1];vn_copy[2]=vn[2];
...
bool Container::computeClosestColl(std::vector<Vertex*> &verts,std::vector<Edge*> &edges,
  ...
  for(std::vector<Face*>::iterator i=faces.begin();i!=faces.end();i++){
    ...
    if (fabs(num)>DOUBLE_EPSILON) {
      ...
      // add to p if intersection is on face
      // return true if point lies inside or on an edge of face, false otherwise
      double pp[3] = {v->pN[0],v->pN[1],v->pN[2]};
      inside = getPlaneIntersection(*i,pp,fn,num,dot(fn,fn),p);
    }
    // if a point has been found, i.e. inside==true
    if(inside==true){
      // create triplet
...
void Container::computePC(Face *f,Vertex *v,double pC[3]) {
  ...
  if (fabs(num)<DOUBLE_EPSILON) {
    ...
    // return true if point lies inside or on an edge of face, false otherwise
    double pp[3] = {v->pN[0],v->pN[1],v->pN[2]};
    inside = getPlaneIntersection(f,pp,fn,num,dot(fn,fn),p);
    // if a point has been found, i.e. inside==true
    // then it is necessarily the closet, we are done
    // but if a point has not been found, then keep looking
    // closest point must lie on an edge or vertex
...
void Container::computePC(Face *f,const double p[3],double pC[3]) {
  // initialize point class instance with current vertex
  Point pp(p[0],p[1],p[2]);
  // get face vertex coordinates
  double *P[3] = {&(f->v[0])->pN[0],&(f->v[1])->pN[0],&(f->v[2])->pN[0]};
  // compute vector connecting arbitrary face vertex and current vertex
  double diff[3] = {P[0][0]-p[0],P[0][1]-p[1],P[0][2]-p[2]};
  // face normal
  double fn[3];
  f->getNormal(fn);
  // compute indicators
  ...
  if (fabs(num)<DOUBLE_EPSILON) {
    ...
    for(int i=0;i<3;i++){
      for(int j=0;j<3;j++){
        del[i][j]=p[j]-f->v[i]->pN[j];
      }
      // if vector length is small
      if(sqrt(l2(del[i]))<DOUBLE_EPSILON){
        // current vertex is on face vertex
        onVertex=true;
        pp.add(f->v[i]->pN[0],f->v[i]->pN[1],f->v[i]->pN[2]);
      }
    }
    if(onVertex==false){
      // current vertex does NOT lie on face vertex
      ...
      if(onEdge==false){
        if( (sqL[0]>0 && sqL[1]>0 && sqL[2]>0 ) || 
          ...
          // add each face vertex
          for (int i=0;i<3;i++) {
            pp.add(P[i][0],P[i][1],P[i][2]);
          }
          // add points of minimum distance between current vertex and each face edge
          getEdgeIntersection(p,P,pp);
        }
      }
    }
  } else {
    ...
    // add to p if intersection is on face
    // return true if point lies inside or on an edge of face, false otherwise
    double ppp[3] = {p[0],p[1],p[2]};
    inside = getPlaneIntersection(f,ppp,fn,num,dot(fn,fn),pp);
    // if a point has been found, i.e. inside==true
    // then it is necessarily the closet, we are done
    // but if a point has not been found, then keep looking
    // closest point must lie on an edge or vertex
...
bool Container::computeClosestBarycenter(Face *f,double p[3],double &squareD,double vn[3],double q[3]) {
  bool signal = false;
  ...
  vn_copy[0]=vn[0];vn_copy[1]=vn[1];vn_copy[2]=vn[2];
  // compute separation vector
  double sep_vec[3] = {pC[0]-p[0],pC[1]-p[1],pC[2]-p[2]};
