...
{
public:
  std::vector<double> ecw;  // extracellular widths
  std::ofstream Sfile;
  std::ofstream Cfile;
  std::ofstream Olist;
  ...
  void computePC(Face*,const double[3],double[3]);
  void fileOutit(void);
  void sepFileOutit(void);
  double computeECW(Space&);
  void computeECWstatistics(void);
  double computeEVF(void);
  //	void updateFile(int,bool,double,double);
  void updateFile(int,bool,double,double,double);
  ...
  bool findClosestBarycenter(Space&,double[3],double[3],Face*);
  bool computeClosest(Face*,Vertex*,double&,double[3]);
  bool computeClosestBarycenter(Face*,double[3],double&,double[3],double[3]);
  bool computeClosestColl(std::vector<Vertex*>&,std::vector<Edge*>&,std::vector<Face*>&,
                          Vertex*,double&,double[3]);
  bool computeClosestCollBarycenter(std::vector<Vertex*>&,std::vector<Edge*>&,std::vector<Face*>&,
                          double[3],double&,double[3],double[3],Face*);
  bool getPlaneIntersection(Face*,double[3],double*,double,double,Point&);
  //void getEdgeIntersection(Vertex*,double*[3],Point&);
  void getEdgeIntersection(const double[3],double*[3],Point&);
  void getBoxes(vector<Box*>&,Vertex*,Space&);
  void getCandidateFaces(vector<Box*>&,Vertex*,std::vector<Face*>&);
  void getCandidateFacesBarycenter(vector<Box*>&,Face*,std::vector<Face*>&);
  ...
  bool searchCPBarycenter(double[3],Face*,std::vector<Face*>&,double&,double[3]);
  //void getPD2(double[6],double[6],Vertex*);
  void getPD2(double[6],double[6],double[3]);
  //
  ...
  Face* getNonAdjacentFaceOfVertexBarycenter(Face*,Vertex*);
  Face* getNonAdjacentFaceOfEdge(Vertex*,Edge*);
  Face* getNonAdjacentFaceOfEdgeBarycenter(Face*,Edge*);
...
Space::Space(Container& c){
  cerr << "Initializing space data structure..............";
  cerr.flush();	
  boundWorld(c);
  initBoxes();
  //fileInit();
...
public:
  ...
  void writeFilesAndUpdateMaxGain(Container&,int,Space&);
...
void Monitor::writeFilesAndUpdateMaxGain(Container &c,const int group,Space &s)
{
  // compute median extracellular width and extracellular volume fraction
  double median_ecw = c.computeECW(s);
  double evf = c.computeEVF();

  gettimeofday(&tim,NULL);
  double t2=tim.tv_sec+(tim.tv_usec/1000000.0);
  //	c.updateFile(group,count>1,t2-t1,max_gain);
  c.updateFile(group,count>1,t2-t1,median_ecw,evf);
...
public:
  ...
  double uv_vert2[2];
  Grid(Face*);
  void computeBarycenter(double[3],int,int,Face*);
};
