...
extern FILE *yyin;
struct object *op;
struct polyhedron *input_mesh,*tagging_mesh;
  ...
  char *argv[];  
{
  ...
  tagging_mesh_filename=argv[2];
  sprintf(fully_outside_filename,"fully_outside.mdl");
  sprintf(fully_inside_filename,"fully_inside.mdl");
  sprintf(on_edge_filename,"on_edge.mdl");

  if ((volume=(struct volume *)malloc(sizeof(struct volume)))==NULL) {
    fprintf(stderr,"mesh_tag_region: cannot store volume data\n");
    exit(1);
  }      
  if ((volume->x_partitions=(double *)malloc
       ((4+NUM_DICES)*sizeof(double)))==NULL) {
    fprintf(stderr,"mesh_tag_region: cannot store volume partitions\n");
    exit(1);
  }      
  ...
  line_num=0;
  if (yyparse()) {
    fprintf(stderr,"mesh_tag_region: error parsing file: %s\n",
            input_mesh_filename);
    exit(1);
  } 
  fclose(yyin);
  ...
  if ((yyin=fopen(tagging_mesh_filename,"r"))==NULL) {
    fprintf(stderr,"mesh_tag_region: error opening file: %s\n",
  ...
  fclose(yyin);
  tagging_mesh=(struct polyhedron *)op->contents;

  pos1=tagging_mesh->llf.x-(2*EPSILON_2);
  pos2=tagging_mesh->urb.x+(2*EPSILON_2);
  ...
  pos1=tagging_mesh->llf.y-(2*EPSILON_2);
  pos2=tagging_mesh->urb.y+(2*EPSILON_2);
  part_delta=(pos2-pos1)/(NUM_DICES+1);
  y_delta=part_delta;
  ...
  pos1=tagging_mesh->llf.z-(2*EPSILON_2);
  pos2=tagging_mesh->urb.z+(2*EPSILON_2);
  part_delta=(pos2-pos1)/(NUM_DICES+1);
  z_delta=part_delta;
  ...
  struct volume *volp;
{
  ...
  if ((volp->x_walls=(struct wall **)malloc
    ...
    fprintf(stderr,"mesh_tag_region: Cannot store subvolume partition data\n");
    return(1);
  }
  ...
  for (i=0;i<nx_parts;i++) {
    ...
    if ((wp=init_wall(face,NULL,4))==NULL) {
      fprintf(stderr,"mesh_tag_region: cannot store wall data");
      return(1);
    }
  ...
  struct wall *wp;
{
  ...
  while (wp!=NULL) {
    ...
    if (umin_part<0) {
      ...
      printf("mesh_tag_region warning: to avoid coincidence, X partition at %1.15g repositioned to %1.15g\n",pos,new_pos);
    ...
    if (vmin_part<0) {
      ...
      new_pos=pos-(2.0*EPSILON_2);
      printf("mesh_tag_region warning: to avoid coincidence, Y partition at %1.15g repositioned to %1.15g\n",pos,new_pos);
    ...
    if (wmax_part<0) {
      ...
      new_pos=pos+(2.0*EPSILON_2);
      printf("mesh_tag_region warning: to avoid coincidence, Z partition at %1.15g repositioned to %1.15g\n",pos,new_pos);
    ...
    if (reposition) {
      ...
      printf("mesh_tag_region: Restarting volume decomposition process...\n");
      reposition=0;
      wp=wp_start;
    }
    else {
...
int clip_mesh(volp,php,input_mesh_filename,tagging_mesh_filename)
  struct volume *volp;
  struct polyhedron *php;
  char *input_mesh_filename;
  char *tagging_mesh_filename;
