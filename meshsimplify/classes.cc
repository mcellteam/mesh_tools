...
{
public:
	std::vector<Vertex*> v;	// vertices to be deleted
	std::vector<Face*> f;	// faces to be deleted
	Vertex *v1,*v2;			// third vertices NOT to be deleted
	std::vector<Vertex*> t;	// t = third vertices
	Edge *e;				// associated edge
	bool flag;				// switch between writing v1 and v2
	Triangle(Edge*);
	void gatherTriangleVerts(void);
	void deleteTriangleVerts(void_list*&);
	void deleteTriangleFaces(void_list*&);
	void findThirdVertices(void);
};

Triangle::Triangle(Edge *E){
	v1=v2=NULL;
	e=E;
	flag=false;
}
...
{
public:
	std::vector<Edge*> ec;	// edge* collection
	std::vector<Face*> fc;	// face* collection	
};
