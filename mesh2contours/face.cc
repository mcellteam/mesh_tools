...
void Face::getzEM (Vertex * & aa,Vertex * & bb,const double & zEM)
{
  aa=bb=NULL;
  if (distinguishable(v[0]->getpN(2),zEM)==false){aa=v[0];}
  if (distinguishable(v[1]->getpN(2),zEM)==false)
  {
    if(aa==NULL){aa=v[1];}else{bb=v[1];}
  }
  if (distinguishable(v[2]->getpN(2),zEM)==false)
  {
    if (bb!=NULL)
    {
      cout << "\nFace::getzEM: "
