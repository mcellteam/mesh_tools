...
Object::Object (std::string s)
  ...
  bad_angle(),bad_length(),zero_area(),contours()
...
//void Object::print (Controls &cs)
void Object::printContours (void)
{
...
void Object::addContour (vec_f & fg,const double & zEM)
{
  // instantiate contour
  Contour *cc;
  cc=new Contour();
...
void Object::findContours(const double & zEM)
{
  vec_f fg;
  // for each face in object
  for (std::vector<Face*>::iterator i=f.begin();i!=f.end();i++)
  {
    // find faces with exactly two vertices at z==zEM
    int k=0;
    for (int j=0;j<3;j++)
    {
      if (distinguishable((*i)->ptr_vertex(j)->getpN(2),zEM)==false){k++;}
    }
