...
std::string Controls::getUsageMessage (void)
{
  ...
  message=message+
        "NAME\n"+
        "       mesh2contours - mesh quality analyzer\n"+
        "\nSYNOPSIS\n"+
        "       mesh2contours [options] FILE|DIR\n"+
        "\nDESCRIPTION\n"+
        "       mesh2contours is a general purpose mesh analyzer useful for\n"+
        "       generating a complete summary of the current state of a mesh.\n"+
        "       mesh2contours assesses mesh integrity (e.g. missing data),\n"+
        ...
        "       mesh2contours -b 10.0 -c 1.0 -p filename\n"+
        "              Evaluate mesh integrity, attributes, and characteristics \n"+
        "              for the single mesh file, print the results, and print \n"+
        "              the mesh elements preventing the mesh from being good with\n"+
        "              regards to the mesh characteristics and the attributes, if any.\n"+
        ...
        "       -z\n"+
        "              Expected z value for which contours are to extracted.\n"+
...
void Controls::parse(int argc,char **argv,std::string message)
{
  ...
  opterr=0;
  char *eptr=NULL;
  while((c=getopt(argc,argv,"ab:c:d:e:f:hipqvz:")) != -1)
    switch(c)
    {
      case 'a':
