  ...
  char *argv[];  
{
  char *argstr;
	...
	if (argc<2) {
      	  fprintf(stderr,"\nRead NetGen volume mesh file and convert to TetGen surface mesh format.\n ");
	...
	if ((netgenin=fopen(infile_1,"r"))==NULL) {
	  fprintf(stderr,"netgen2smesh: error opening volume file: %s\n",infile_1);
	  exit(1);
	} 
        curr_file=infile_1;
	fflush(stdout);
	if (netgenparse()) {
	  fprintf(stderr,"netgen2smesh: error parsing volume file %s\n",curr_file);
	  exit(1);
	} 
	fclose(netgenin);
