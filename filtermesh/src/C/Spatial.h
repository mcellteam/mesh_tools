...
class Spatial {			// abstract class
  ...
  protected:
	...
	int float2index(double fd) const;
	double index2float(int i) const;
...
class PointSpatial : public Spatial {
  ...
  private:
	...
	Map<int,Stack<Node*>*> map; // encoded cube index -> Stack
};

class ObjectSpatial : public Spatial {
  public:
	typedef double (*DISTF)(const Point& p, Univ id);
  ...
  protected:
	...
	Univ pqid(Univ pqe) const;
  private:
	Map<int,Stack<Univ>*> map; // encoded cube index -> Stack
	DISTF approxf2;
	DISTF exactf2;
};
