  ...
  char *argv[];  
{
  struct surface_mesh *smp;
	...
	if ((world_obj=(struct object *)malloc(sizeof(struct object)))==NULL) {
	  printf("vrml2mesh: cannot store world object\n");
	  exit(1);
        }
        world_obj->next=NULL;
	...
	if ((yyin=fopen(infile,"r"))==NULL) {
	  printf("vrml2mesh: error opening file: %s\n",infile);
	  exit(1);
	} 
	fflush(stdout);
	if (yyparse()) {
	  printf("vrml2mesh: error parsing file: %s\n",infile);
	  exit(1);
	} 
	fclose(yyin);
...
int analyze_surface_mesh(struct surface_mesh *smp)
{
  ...
  if ((aspect_ratio_histogram=(struct histogram *)malloc(sizeof(struct histogram)))==NULL) {
    printf("vrml2mesh: out of memory while creating aspect ratio histogram\n");
    return(1);
  }
