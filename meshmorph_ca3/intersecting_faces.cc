 ...
 * \param[in] face Face of interest.
 * \param[in] record_intersections If true then detect and record face intersections.
 * If false, then just detect face intersections.
 * \return True if new face intersections are detected; false otherwise.
 */

bool Intersecting_Faces::detectNewFaceInts (Face * const face, bool record_intersections)
{
...
bool Intersecting_Faces::vertAdjFacesHaveNewInt (Vertex const * const v)
{
  ...
  for (fp_cit i=v->begin();i!=v->end();++i)
  {
    ...
    //  'false' means do not record face intersections, only detect
    if (detectNewFaceInts(*i,false)==true){return true;}
  }
                                                 ...
                                                 hashset_v & vertices)
{
  ...
  for (fp_cit k=v->begin();k!=v->end();++k)
  {
    ...
    // true means detect and record new face intersections
    if (detectNewFaceInts(*k,true))
    {
      // for each intersecting face of adjacent face
      vec_fp *fv=getIntersectingFacesRHS(*k);
      for (fp_it p=(*fv).begin();p!=(*fv).end();++p)
                                            ...
                                            Face const * const of) const
{
  ...
  // DEBUG
//  if (cf->isMatch(5406,"d01")==true && of->isMatch(39657,"d01")==true)
//  {
//    cout << "\nIntersecting_Faces::checkFaceFaceInts: num_unique = "
//          << num_unique << endl;
//  }
  // DEBUG
...
//            << "tri_tri_intersect will be called.\n" << endl;
//      target_flag = true;
//    }
    // DEBUG
    //return NoDivTriTriIsect(cv0,cv1,cv2,ov0,ov1,ov2,target_flag);
    return tri_tri_intersect(cv0,cv1,cv2,ov0,ov1,ov2,target_flag);
  }
}
