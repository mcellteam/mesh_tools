...
bool Refractory::vertexIsMoveCandidate (Vertex * const v)
{
  ...
  return ((v->getClosestFace()!=NULL) || (Nice::instance().vertexIsNice(v)==false))
        && (isRefracted(v)==false)
...
void Refractory::updateSuccessfulMove (const int & group,Vertex * const v)
{
  ...
  if (target!=vert_move_distr.end())
  {
    ...
    if ((*target).second > Controls::instance().get_max_touches())
    {
    ...
    else
    {
      ...
      v->setLastRefractoryIter(Log::instance().getN()+Controls::instance().get_refractory_period());
    }
  }
  else
  {
